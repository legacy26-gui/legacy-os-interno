...
import { createFixedExpense } from "@/lib/actions/financeiro";
...
export function FixedExpenseForm() {
  const [state, formAction, pending] = useActionState(createFixedExpense, undefined);
  ...
  return (
    ...
    >
      <div className="grid sm:grid-cols-4 gap-2">
        <input name="description" placeholder="Descrição (ex: Aluguel do escritório)" required className={inputClass} />
        ...
        <input name="dueDay" type="number" min="1" max="31" placeholder="Dia do mês (opcional)" className={inputClass} />
      </div>
      ...
      >
        <Plus size={14} />
        {pending ? "Salvando..." : "Adicionar despesa fixa"}
      </button>
    </form>
  );
}
