...
export function CampaignForm({ clients }: { clients: { id: string; companyName: string }[] }) {
  ...
  return (
    ...
    >
      <div className="grid sm:grid-cols-3 lg:grid-cols-6 gap-2">
        ...
        <input name="periodStart" type="date" required className={inputClass} title="Início" />
        <input name="periodEnd" type="date" required className={inputClass} title="Fim" />
      ...
      >
        <Plus size={14} />
        {pending ? "Salvando..." : "Lançar campanha"}
      </button>
    </form>
  );
}
