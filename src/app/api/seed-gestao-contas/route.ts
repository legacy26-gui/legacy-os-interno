  ...
  for (let i = 0; i < clients.length; i++) {
    ...
    await prisma.dailyReview.create({
      data: {
        ...
        checkedBillingLimit: p.label !== "critico",
        checkedPendingPayments: true,
        checkedWhatsappResolved: p.label === "saudavel",
    ...
    await prisma.weeklyReview.create({
      data: {
        clientId: c.id,
        reviewerId: managerId,
        paymentCleared: p.label === "saudavel",
        reportGenerated: true,
        checkedBestCampaigns: p.label !== "critico",
        checkedWeeklyCost: true,
        definedNewCreatives: p.label === "saudavel",
        definedNewCampaigns: p.label === "saudavel",
